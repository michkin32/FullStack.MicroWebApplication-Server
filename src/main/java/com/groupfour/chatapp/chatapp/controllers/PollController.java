...
public class PollController {
    ...
    public ResponseEntity<?> createPoll(@Valid @RequestBody Poll poll) {
        ...
        public ResponseEntity<?> updatePoll (@Valid @RequestBody Poll poll, @PathVariable Long id, @PathVariable Long
        pollId){
            return new ResponseEntity<>(pollService.update(pollId, poll), HttpStatus.CREATED);

            @RequestMapping(value = "/chat/{id}/polls", method = RequestMethod.PUT)
            public ResponseEntity<?> updatePoll (@RequestBody Poll poll, @PathVariable Long id, @PathVariable Long
            pollId){
                return new ResponseEntity<>(pollService.update(pollId, poll), HttpStatus.CREATED);
            }
